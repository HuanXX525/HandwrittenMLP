...
def train(optiName, lr, epoches, numOfTrain=None):
    ...
    # 模型参数
    numInputs, numOutputs, numHiddens1, numHiddens2, numHiddens3 = 784, 10, 256, 128, 64
    batch_size = args.batchSize

    # 初始化模型和优化器
    if not args.continueTrain:
        L1 = Layer(numInputs, numHiddens1)
        L2 = Layer(numHiddens1, numHiddens2)
        L3 = Layer(numHiddens2, numHiddens3)
        L4 = Layer(numHiddens3, numOutputs)
    else:
        L1 = Layer.load_model(f'{optiName}L1.pkl')
        L2 = Layer.load_model(f'{optiName}L2.pkl')
        L3 = Layer.load_model(f'{optiName}L3.pkl')
        L4 = Layer.load_model(f'{optiName}L4.pkl')
    ...
    for epoch in range(epoches):
        ...
        for batch_idx in range(total_batches):
            ...
            for img, label in zip(batch_images, batch_labels):
                ...
                a3 = L3(ActivationFunction.RelU(a2))
                a4 = L4(ActivationFunction.RelU(a3))
                yHat = ActivationFunction.softmax(a4)
    ...
    L3.save_model(f'{optiName}L3.pkl')
    L4.save_model(f'{optiName}L4.pkl')
