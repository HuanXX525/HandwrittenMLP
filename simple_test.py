from backward import ActivationFunction, LossFunction, Optimizer, to_one_hot, Layer, Value
import numpy as np
...
def simple_test():
    ...
    # 创建网络层
    L1 = Layer(input_size, hidden_size, init_type="xavier", activation="relu")
    L2 = Layer(hidden_size, output_size, init_type="xavier", activation="relu")
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(len(train_X)):
            ...
            # 计算损失
            loss = LossFunction.categorical_cross_entropy(y_hat, train_y[i])
    ...
    for i in range(len(train_X)):
        ...
        # 打印预测结果和真实标签
        predicted_class = np.argmax([v.data for v in y_hat])
        ...
        # 打印概率分布
        probs = [v.data for v in y_hat]
        print(f"  预测概率分布: {[f'{p:.4f}' for p in probs]}")
        print(f"  真实标签: {[int(v) for v in train_y[i]]}")
